...
def test_validate_and_get_dataset_names_works():
    ...
    assert dbtool.validate_and_get_dataset_names(["oath_hearings"]) == ["oath_hearings"]
...
def test_rowcounts_works(test_db_env, capsys):
    subprocess.check_call(
        ["python", "load_dataset.py", "hpd_registrations"], env=test_db_env
    )
    capsys.readouterr()
...
def test_list_lastmod_works(db, capsys):
    ...
    with load_dbhash() as dbhash:
        info = LastmodInfo(
            url, etag="blah", last_modified="Tue, 01 Jan 2019 20:56:28 UTC"
        )
